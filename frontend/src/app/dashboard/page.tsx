  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: "900px", margin: "0 auto", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div style={{ position: "relative" }}>
            ...
            <span style={{ position: "absolute", top: "8px", right: "8px", width: "7px", height: "7px", background: "#6366f1", borderRadius: "50%", border: "1.5px solid #0a0a0f" }}></span>
          </div>
          <div style={{ width: "38px", height: "38px", borderRadius: "10px", background: "linear-gradient(135deg, #6366f1, #8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "14px", fontWeight: 700 }}>
      ...
      {user?.kyc_status !== "approved" && (
        <div style={{ display: "flex", alignItems: "center", gap: "14px", background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: "14px", padding: "14px 18px", marginBottom: "20px" }}>
          <span style={{ fontSize: "22px" }}>🛡️</span>
          <div style={{ flex: 1 }}>
            <p style={{ color: "#fbbf24", fontSize: "13px", fontWeight: 600, margin: 0 }}>Complete your KYC verification</p>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "11px", marginTop: "2px" }}>Unlock higher transaction limits and full platform access</p>
          </div>
          <button style={{ padding: "7px 16px", background: "rgba(245,158,11,0.15)", color: "#fbbf24", border: "1px solid rgba(245,158,11,0.3)", borderRadius: "8px", fontSize: "12px", cursor: "pointer", fontWeight: 500, whiteSpace: "nowrap" }}>
            Verify Now →
          </button>
        </div>
      )}
      ...
      {/* Message */}
      {msg && (
        <div style={{ borderRadius: "12px", padding: "13px 16px", marginBottom: "20px", display: "flex", alignItems: "center", gap: "10px", background: msg.type === "success" ? "rgba(16,185,129,0.1)" : "rgba(239,68,68,0.1)", border: `1px solid ${msg.type === "success" ? "rgba(16,185,129,0.2)" : "rgba(239,68,68,0.2)"}`, color: msg.type === "success" ? "#34d399" : "#f87171", fontSize: "13px", fontWeight: 500 }}>
      ...
      {/* Wallet Card */}
      <div style={{ background: "linear-gradient(135deg, #1e1b4b 0%, #2d2a6e 50%, #1e1b4b 100%)", borderRadius: "20px", padding: "28px", marginBottom: "20px", border: "1px solid rgba(99,102,241,0.25)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative" }}>
          <p style={{ color: "rgba(165,180,252,0.6)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "1px", fontWeight: 500, margin: 0 }}>Total wallet balance</p>
          <p style={{ color: "#fff", fontSize: "40px", fontWeight: 700, margin: "10px 0 4px", letterSpacing: "-1px", lineHeight: 1 }}>
            {wallet ? fmt(wallet.balance) : "—"}
          </p>
          <p style={{ color: "rgba(165,180,252,0.4)", fontSize: "12px", marginBottom: "22px" }}>INR • Active • Updated just now</p>
          <div style={{ display: "flex", gap: "10px" }}>
            <button onClick={() => { setShowAddMoney(!showAddMoney); setMsg(null); }} style={{ padding: "9px 18px", background: "#6366f1", color: "#fff", border: "none", borderRadius: "10px", fontSize: "13px", fontWeight: 500, cursor: "pointer" }}>
              + Add Money
            </button>
            <button onClick={() => router.push("/dashboard/withdraw")} style={{ padding: "9px 18px", background: "rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.8)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "10px", fontSize: "13px", fontWeight: 500, cursor: "pointer" }}>
              ↓ Withdraw
            </button>
            <button style={{ padding: "9px 18px", background: "rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.8)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "10px", fontSize: "13px", fontWeight: 500, cursor: "pointer" }}>
              ⊞ QR Pay
            </button>
          </div>
        </div>
        ...
        <div style={{ background: "#13131f", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "18px", padding: "20px", marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", marginBottom: "12px" }}>
            {["Razorpay", "Cashfree", "Simulate (Dev)"].map((label, i) => (
              <button key={label} onClick={() => setPayMode(i)} style={{ padding: "7px 16px", borderRadius: "8px", fontSize: "12px", fontWeight: 500, background: payMode === i ? "#6366f1" : "rgba(255,255,255,0.05)", color: payMode === i ? "#fff" : "rgba(255,255,255,0.4)", border: "none", cursor: "pointer" }}>{label}</button>
            ))}
          </div>
          <div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
            <input type="number" value={addAmount} onChange={(e) => setAddAmount(e.target.value)} placeholder="Enter amount" style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", padding: "12px 16px", color: "#fff", fontSize: "14px", outline: "none" }} />
            <button onClick={handleAddMoney} disabled={adding} style={{ padding: "12px 24px", background: adding ? "rgba(99,102,241,0.5)" : "#6366f1", color: "#fff", border: "none", borderRadius: "12px", fontSize: "13px", fontWeight: 600, cursor: adding ? "not-allowed" : "pointer" }}>
              {adding ? "Processing..." : "Pay Now"}
            </button>
          </div>
          <div style={{ display: "flex", gap: "8px" }}>
            {[100, 500, 1000, 2000].map((amt) => (
              <button key={amt} onClick={() => setAddAmount(amt.toString())} style={{ padding: "6px 14px", background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "8px", fontSize: "12px", cursor: "pointer" }}>₹{amt}</button>
            ))}
          </div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "20px" }}>
        {quickActions.map((action) => (
          <button key={action.label} onClick={() => router.push(action.href)} style={{ background: "#13131f", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "18px 12px", display: "flex", flexDirection: "column", alignItems: "center", gap: "10px", cursor: "pointer" }}>
          ...
          transactions.map((tx) => (
            <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: "14px", padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ color: "#fff", fontSize: "13px", fontWeight: 500, margin: 0, textTransform: "capitalize" }}>{tx.category.replace(/_/g, " ")}</p>
              ...
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "11px", marginTop: "2px", textTransform: "capitalize" }}>{tx.transaction_status}</p>
              </div>
            </div>
          ))
        )}
